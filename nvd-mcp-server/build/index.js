...
server.tool("get_cve_details", "Retrieve detailed information for a specific CVE ID", { cve_id: z.string().describe("CVE ID in format CVE-YYYY-NNNNN") }, async ({ cve_id }) => {
    const headers = {};
    ...
    if (!vulnerability)
        return { content: [{ type: "text", text: "CVE not found." }] };
    return {
        content: [{
                type: "text",
                text: JSON.stringify({
